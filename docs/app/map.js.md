...
`wq/map.js` is typically imported via [AMD] as `map`, though any local variable name can be used.  As of wq.app 0.8.1, wq/map.js is a formal plugin for [wq/app.js] and can be enabled like this:
...
As of wq.app 0.8.1, `map.init()` is called automatically by `app.init()` with the contents of `config.map`.  The configuration object default settings for rendered maps including initial bounds.

name | default | purpose
-----|---------|---------
`bounds` | `[[-4,-4],[4,4]]` | Default extent for initially rendered map.  As of wq.app 0.8.0, this is specified as a bounds rather than a center and zoom, to ensure the full intended extent is visible regardless of screen size.
...
As of wq.app 0.8.1, the map configuration is split into four sections:
...
`popup` | The name of a popup template to use when rendering marker popups (New in 0.8.1).  See map.renderPopup() below
...
## Map Editing
As of 0.8.1, wq/map.js supports editing layers via the [Leaflet.draw] plugin.  This functionality can be enabled by setting a `draw` attribute on the map's `edit` mode configuration.  The `draw` configuration will be passed on to the Leaflet draw control to enable different drawing types.
...
`map.renderPopup(page)` generates a callback function suitable for use as the `oneach` argument to a layer configuration, or as the [onEachFeature] option in `L.GeoJSON`.  (As of wq.app 0.8.1, a layer configuration of `oneach: map.renderPopup(page)` can be replaced with `popup: page`.)  The function will render a popup using the template with the name `[page]_popup`.  The template should be included among the templates provided to [wq/app.js]' `init()` function.  The template context will be the GeoJSON properties on each feature.  The example map at the top of the page uses the following template:
