 ...
 * A model-driven, `localForage`-backed cache of REST API responses (provided by [wq/store.js]).
...
`baseurl` | (optional) path used to prefix application URLs to help with route matching in [wq/router.js].  `baseurl` should be set to the base url of your application (without the trailing `/`) if your application is not running at the root of your domain.
...
`app.init()` internally calls [router.register()] for each page in the `pages` configuration section.  The callbacks sent to `router.register()` are simple wrappers around `app.go()`.
...
The `debug` option enables console logging in wq/app.js, [wq/store.js] and [wq/router.js].  If specified as a number, `debug` will set the verbosity level in wq/store.js.
