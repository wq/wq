...
wq/app.js also provides a simple plugin API that can be used instead of registering custom `pageshow` events via `router.addRoute()`.  The [wq/map.js] and [wq/photos.js] plugins both use this API.  A wq/app.js plugin is essentially just an object with three properties:
